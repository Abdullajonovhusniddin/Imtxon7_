...
export default function Videolar({ groupId }) {
                    ...
                    <tbody>
                        {videosData.length > 0 ? videosData.map((video, idx) => (
                            <tr key={video.id || idx}>
                                <td>{idx + 1}</td>
                                <td>
                                    <div className={styles.videoNameCell} onClick={() => setSelectedPlayVideo(video)}>
                                        <PlayCircleOutlineRoundedIcon className={styles.playIcon} fontSize="small" />
                                        {video.name || video.originalname || video.title || video.videoName || "Video"}
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td colSpan="8" style={{ textAlign: "center", padding: "32px", color: "#94a3b8" }}>Videolar topilmadi</td>
                            </tr>
                        )}
                    </tbody>
                </table>
                ...
                <div className={styles.modalOverlay} onClick={() => setSelectedPlayVideo(null)}>
                    <div className={styles.videoPlayerModalContent} onClick={(e) => e.stopPropagation()}>
                        <div className={styles.videoPlayerHeader}>
                            <h2>{selectedPlayVideo.name || selectedPlayVideo.originalname || selectedPlayVideo.title || "Video"}</h2>
                        ...
                        <div className={styles.videoPlayerContainer}>
                            <video
                                controls
                                autoPlay
                                src={getFileUrl(selectedPlayVideo.url || selectedPlayVideo.video_url)}
                            />
                        </div>
                    </div>
                </div>
