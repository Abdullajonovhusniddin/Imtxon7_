...
import AddRoundedIcon from '@mui/icons-material/esm/AddRounded.js';;
import EditGroupSidebar from '../../components/UI/ManagementSidebar/EditGroupSidebar';
import GroupsRoundedIcon from '@mui/icons-material/esm/GroupsRounded.js';;
...
import RefreshRoundedIcon from '@mui/icons-material/esm/RefreshRounded.js';;
import ArchiveOutlinedIcon from '@mui/icons-material/esm/ArchiveOutlined.js';;
            ...
            <div className={styles.tableCard}>
                <div className={styles.tableWrapper} style={{ position: 'relative', opacity: isLoading ? 0.6 : 1, transition: 'opacity 0.2s', minHeight: '150px' }}>
                    ...
                    <table className={styles.table}>
                        ...
                        <tbody>
                            {groups.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((group) => (
                                <tr key={group.id} onClick={() => navigate(`/groups/${group.id}`)} style={{ cursor: 'pointer', transition: 'background-color 0.2s' }}>
                                    <td>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', borderTop: '1px solid var(--border)', color: 'var(--text)' }}>
                    <button 
                        onClick={() => setCurrentPage(p => Math.max(1, p - 1))} 
                        disabled={currentPage === 1}
                        style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--border)', background: currentPage === 1 ? 'var(--bg)' : 'var(--surface-strong)', color: currentPage === 1 ? 'var(--text-secondary)' : 'var(--text)', cursor: currentPage === 1 ? 'not-allowed' : 'pointer' }}
                    >
                        Oldingi
                    </button>
                    <span style={{ fontWeight: 500 }}>Sahifa {currentPage} / {Math.ceil(groups.length / itemsPerPage) || 1}</span>
                    <button 
                        onClick={() => setCurrentPage(p => Math.min(Math.ceil(groups.length / itemsPerPage), p + 1))} 
                        disabled={currentPage === Math.ceil(groups.length / itemsPerPage) || groups.length === 0}
                        style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--border)', background: currentPage === Math.ceil(groups.length / itemsPerPage) || groups.length === 0 ? 'var(--bg)' : 'var(--surface-strong)', color: currentPage === Math.ceil(groups.length / itemsPerPage) || groups.length === 0 ? 'var(--text-secondary)' : 'var(--text)', cursor: currentPage === Math.ceil(groups.length / itemsPerPage) || groups.length === 0 ? 'not-allowed' : 'pointer' }}
                    >
                        Keyingi
                    </button>
                </div>
